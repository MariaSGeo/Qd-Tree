...
long mk_my_line_item(DSS_HUGE index, line_t_2 * o, order_t * order, customer_t * customer, supplier_t * supplier, part_t * part,code_t * n,code_t * r,long upd_num){
    srand(time(NULL));
//    int random_index = rand() % range + min;

    mk_order(rand() % 150000 + 1, order, upd_num);
    set_line_item_order(o, order);

    mk_cust(rand() % 150000 + 1, customer);
    set_line_item_customer(o, customer);

    mk_supp(rand() % 10000 + 1, supplier);
    set_line_item_supplier(o, supplier);

    mk_part(rand() % 200000 + 1, part);
    set_line_item_part(o, part);

    mk_nation(rand() % 25 + 1, n);
    set_line_item_supp_nation(o, n);

    mk_nation(rand() % 25 + 1, n);
    set_line_item_cust_nation(o, n);

    mk_region(rand() % 5 + 1, r);
    set_line_item_supp_region(o, r);

    mk_region(rand() % 5 + 1, r);
    set_line_item_cust_region(o, r);
...
void set_line_item_order(line_t_2 * line_item, order_t * order){
    ...
    line_item->orderkey = order->okey;
    line_item->orderstatus = order->orderstatus;
    line_item->ordertotalprice = order->totalprice;
    strcpy(line_item->orderdate, order->odate);
    strcpy(line_item->orderpriority, order->opriority);
    strcpy(line_item->orderclerk, order->clerk);
    line_item->orderspriority = order->spriority;
    strcpy(line_item->ordercomment, order->comment);
    line_item->orderclen = order->clen;

    line_item->lcnt = order->l[0].lcnt;
    line_item->quantity = order->l[0].quantity;
    line_item->eprice = order->l[0].eprice;
    line_item->discount = order->l[0].discount;
    line_item->tax = order->l[0].tax;
    strcpy(line_item->rflag, order->l[0].rflag);
    strcpy(line_item->lstatus, order->l[0].lstatus);
    strcpy(line_item->cdate, order->l[0].cdate);
    strcpy(line_item->sdate, order->l[0].sdate);
    strcpy(line_item->rdate, order->l[0].rdate);
    strcpy(line_item->shipinstruct, order->l[0].shipinstruct);
    strcpy(line_item->shipmode, order->l[0].shipmode);
    strcpy(line_item->comment, order->l[0].comment);
    line_item->clen = order->l[0].clen;

}
void set_line_item_customer(line_t_2 * line_item, customer_t * customer){

    line_item->custkey = customer->custkey;
    strcpy(line_item->custname, customer->name);
    strcpy(line_item->custaddress, customer->address);
    line_item->custalen = customer->alen;
    line_item->custnation_code = customer->nation_code;
    strcpy(line_item->custphone, customer->phone);
    line_item->custacctbal = customer->acctbal;
    strcpy(line_item->custmktsegment, customer->mktsegment);
    strcpy(line_item->custcomment, customer->comment);
    line_item->custclen = customer->clen;

}
void set_line_item_supplier(line_t_2 * line_item, supplier_t * supplier){

    line_item->suppkey = supplier->suppkey;
    strcpy(line_item->suppname, supplier->name);
    strcpy(line_item->suppaddress, supplier->address);
    line_item->suppalen = supplier->alen;
    line_item->suppnation_code = supplier->nation_code;
    strcpy(line_item->suppphone, supplier->phone);
    line_item->suppacctbal = supplier->acctbal;
    strcpy(line_item->suppcomment, supplier->comment);
    line_item->suppclen = supplier->clen;

}
void set_line_item_part(line_t_2 * line_item, part_t * part){

    line_item->partkey = part->partkey;
    strcpy(line_item->partname, part->name);
    line_item->partnlen = part->nlen;
    strcpy(line_item->partmfgr, part->mfgr);
    strcpy(line_item->partbrand, part->brand);
    strcpy(line_item->parttype, part->type);
    line_item->parttlen = part->tlen;
    line_item->partsize = part->size;
    strcpy(line_item->partcontainer, part->container);
    line_item->partretailprice = part->retailprice;
    strcpy(line_item->partcomment, part->comment);
    line_item->partclen = part->clen;
    line_item->partsuppqty = part->s[0].qty;
    line_item->partsuppcost = part->s[0].scost;
    strcpy(line_item->partsuppcomment, part->s[0].comment);
    line_item->partsupplen = part->s[0].clen;

}

void set_line_item_supp_nation(line_t_2 * line_item, code_t * n){

    line_item->suppnationkey = n->code;
    strcpy(line_item->suppnationname, n->text);
    strcpy(line_item->suppnationcomment, n->comment);
    line_item->suppnationclen = n->clen;


}

void set_line_item_supp_region(line_t_2 * line_item, code_t * r){

    line_item->suppregionkey = r->code;
    strcpy(line_item->suppregionname, r->text);
    strcpy(line_item->suppregioncomment, r->comment);
    line_item->suppregionclen = r->clen;


}

void set_line_item_cust_nation(line_t_2 * line_item, code_t * n){

    line_item->custnationkey = n->code;
    strcpy(line_item->custnationname, n->text);
    strcpy(line_item->custnationcomment, n->comment);
    line_item->custnationclen = n->clen;

}

void set_line_item_cust_region(line_t_2 * line_item, code_t * r){

    line_item->custregionkey = r->code;
    strcpy(line_item->custregionname, r->text);
    strcpy(line_item->custregioncomment, r->comment);
    line_item->custregionclen = r->clen;
