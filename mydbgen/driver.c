...
{
	static order_t o;
	static line_t_2 my_line_item_o;
	supplier_t supp;
	customer_t cust;
	part_t part;
	code_t code;
...
//	    }
		...
		{
		case LINE:
            mk_my_line_item(i, &my_line_item_o, &o, &cust, &supp, &part, &code, &region, upd_num);
...
//                    }
            ...
            if (set_seeds == 0)
                tdefs[tnum].loader(&my_line_item_o, i);
            break;
		case ORDER:
  		case ORDER_LINE: 
		...
		if ((table & (1 << i)) && (i==LINE))
		{
			...
			else
			{
				...
				gen_tbl ((int)i, minrow, rowcnt, upd_num);
					fprintf (stderr, "Done Generating data for %s", tdefs[i].comment);
				if (verbose > 0)
					fprintf (stderr, "done.\n");
			}
		}
