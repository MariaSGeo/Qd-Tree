...
int pr_mylineitem (line_t_2 * l, int mode) {
    ...
    fp_l = fopen( "my_line_item.tbl", "a" );

    PR_STRT(fp_l);

    PR_HUGE(fp_l, &l->lcnt);
    PR_HUGE(fp_l, &l->quantity);
    PR_MONEY(fp_l, &l->eprice);
    PR_MONEY(fp_l, &l->discount);
    PR_MONEY(fp_l, &l->tax);
    PR_CHR(fp_l, &l->rflag[0]);
    PR_CHR(fp_l, &l->lstatus[0]);
    PR_STR(fp_l, l->sdate, DATE_LEN);
    PR_STR(fp_l, l->cdate, DATE_LEN);
    PR_STR(fp_l, l->rdate, DATE_LEN);
    PR_STR(fp_l, l->shipinstruct, L_INST_LEN);
    PR_STR(fp_l, l->shipmode, L_SMODE_LEN);
    PR_VSTR(fp_l, l->comment,l->clen);

    //order
    PR_HUGE(fp_l, &l->orderkey);
    PR_CHR(fp_l, &l->orderstatus);
    PR_MONEY(fp_l, &l->ordertotalprice);
    PR_STR(fp_l, l->orderdate, DATE_LEN);
    PR_STR(fp_l, l->orderpriority, O_OPRIO_LEN);
    PR_STR(fp_l, l->orderclerk, O_CLRK_LEN);
    PR_INT(fp_l, l->orderspriority);
    PR_VSTR(fp_l, l->ordercomment, l->orderclen);

    //customer
    PR_HUGE(fp_l, &l->custkey);
    if (scale <= 3000)
        PR_VSTR(fp_l, l->custname, C_NAME_LEN);
    else
        PR_VSTR(fp_l, l->custname, C_NAME_LEN + 3);
    PR_VSTR(fp_l, l->custaddress, l->custalen);
//    PR_HUGE(fp_l, &l->custnation_code);
    PR_STR(fp_l, l->custphone, PHONE_LEN);
    PR_MONEY(fp_l, &l->custacctbal);
    PR_STR(fp_l, l->custmktsegment, C_MSEG_LEN);
    PR_VSTR(fp_l, l->custcomment, l->custclen);

    //supplier
    PR_HUGE(fp_l, &l->suppkey);
    PR_STR(fp_l, l->suppname, S_NAME_LEN);
    PR_VSTR(fp_l, l->suppaddress, l->suppalen);
//    PR_HUGE(fp_l, &l->suppnation_code);
    PR_STR(fp_l, l->suppphone, PHONE_LEN);
    PR_MONEY(fp_l, &l->suppacctbal);
    PR_VSTR(fp_l, l->suppcomment, l->suppclen);

    //PART
    PR_HUGE(fp_l, &l->partkey);
    PR_VSTR(fp_l, l->partname,l->partnlen);
    PR_STR(fp_l, l->partmfgr, P_MFG_LEN);
    PR_STR(fp_l, l->partbrand, P_BRND_LEN);
    PR_VSTR(fp_l, l->parttype, l->parttlen);
    PR_HUGE(fp_l, &l->partsize);
    PR_STR(fp_l, l->partcontainer, P_CNTR_LEN);
    PR_MONEY(fp_l, &l->partretailprice);
    PR_VSTR(fp_l, l->partcomment,l->partclen);

    PR_HUGE(fp_l, &l->partsuppqty);
    PR_MONEY(fp_l, &l->partsuppcost);
    PR_VSTR_LAST(fp_l, l->partsuppcomment, l->partsupplen);

    //CUST NATION
    PR_HUGE(fp_l, &l->custnationkey);
    PR_STR(fp_l, l->custnationname, NATION_LEN);
    PR_VSTR(fp_l, l->custnationcomment, l->custnationclen);

    //CUST REGION
    PR_HUGE(fp_l, &l->custregionkey);
    PR_STR(fp_l, l->suppnationname, REGION_LEN);
    PR_VSTR(fp_l, l->suppnationcomment, l->suppnationclen);

    //SUPP NATION
    PR_HUGE(fp_l, &l->suppnationkey);
    PR_STR(fp_l, l->custnationname, NATION_LEN);
    PR_VSTR(fp_l, l->custnationcomment, l->custnationclen);

    //SUPP REGION
    PR_HUGE(fp_l, &l->suppregionkey);
    PR_STR(fp_l, l->suppregionname, REGION_LEN);
    PR_VSTR(fp_l, l->suppregioncomment, l->suppregionclen);
